...
if [ ! -f "./witness/witness" ]; then
  echo "Witness binary not found at ./witness/witness. Building..."
  make witness
fi
...
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/plain-attestation.json" -- cat "$DEMO_DIR/plain-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on base64-encoded secret..."
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/base64-attestation.json" -- cat "$DEMO_DIR/base64-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on double base64-encoded secret..."
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/double-base64-attestation.json" -- cat "$DEMO_DIR/double-base64-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on URL-encoded secret..."
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/url-encoded-attestation.json" -- cat "$DEMO_DIR/url-encoded-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on hex-encoded secret..."
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/hex-encoded-attestation.json" -- cat "$DEMO_DIR/hex-encoded-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on mixed encoding secret..."
echo "==============================================================="
./witness/witness run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/mixed-encoding-attestation.json" -- cat "$DEMO_DIR/mixed-encoding-secret.txt"
