...
const (
	PinBasePath = "/sys/fs/bpf/witness-nettrace"

	PinPathOrigDstMap       = PinBasePath + "/orig_dst_map"
	PinPathOrigDstMapV6     = PinBasePath + "/orig_dst_map_v6"
	PinPathPIDAllowlist     = PinBasePath + "/pid_allowlist"
	PinPathCommAllowlist    = PinBasePath + "/comm_allowlist"
	PinPathCgroupAllowlist  = PinBasePath + "/cgroup_allowlist"
	PinPathInjectionTimeMap = PinBasePath + "/injection_time_map"
	PinPathTupleCookieMap   = PinBasePath + "/tuple_to_cookie_map"
	PinPathTupleCookieMapV6 = PinBasePath + "/tuple_to_cookie_map_v6"

	PinPathConnect4 = PinBasePath + "/progs/connect4"
	PinPathConnect6 = PinBasePath + "/progs/connect6"
	PinPathSockops  = PinBasePath + "/progs/sockops"

	PinPathLinkConnect4 = PinBasePath + "/links/connect4"
	PinPathLinkConnect6 = PinBasePath + "/links/connect6"
	PinPathLinkSockops  = PinBasePath + "/links/sockops"
)
...
func UnloadWithoutError() {
	unloadErr := Unload()
	if unloadErr != nil {
		log.Errorf("unload error: %v, manually unload programs using: rm -rf /sys/fs/bpf/witness-nettrace", unloadErr)
	}
}
...
func Unload() error {
	...
	for _, dir := range []string{
		...
		if err := os.Remove(dir); err != nil && !os.IsNotExist(err) {
			// Directory should be empty by now, record this error
