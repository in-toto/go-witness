...
func TestDirHash(t *testing.T) {
	...
	// The artifact key and the hashed file names are normalized to forward
	// slashes in RecordArtifacts, so this value is identical on every OS.
...
func TestDirHashWithSymlink(t *testing.T) {
	...
	if err := os.Symlink(testDir2, symlinkDir); err != nil {
		// Creating symlinks on Windows requires elevated privileges or
		// Developer Mode. Skip rather than fail when the OS won't allow it.
	...
	// Below command is example usage on the above created scenario for testdir.
	// find . -type f | cut -c3- | LC_ALL=C sort | xargs -r sha256sum | sha256sum
	// testdir contains no regular files, so this is the SHA256 of empty content
	// and is identical on every OS.
