...
# Test successful pipeline where artifacts match across all steps
...
echo "=== Test: Successful Pipeline ==="
echo ""

# Setup test directory
TEST_DIR="${SCRIPT_DIR}/tmp-success"
rm -rf "${TEST_DIR}"
mkdir -p "${TEST_DIR}"
cd "${TEST_DIR}"
...
# Step 2: Test (uses the app binary from build)
echo "4. Running test step..."
"${SCRIPT_DIR}/witness" run \
    --step test \
    --attestations material,product \
    --signer-file-key-path build-key.pem \
...
# Step 3: Package (uses the app binary from build)
echo "5. Running package step..."
"${SCRIPT_DIR}/witness" run \
    --step package \
    --attestations material,product \
    --signer-file-key-path build-key.pem \
...
# Create Rego policy for package step
PACKAGE_REGO=$(cat <<'REGO' | base64 | tr -d '\n'
package material
...
deny[msg] {
    ...
    msg := "package input doesn't match build output"
}
REGO
)
...
if "${SCRIPT_DIR}/witness" verify \
    ...
    echo ""
    echo "SUCCESS: All steps verified with cross-step attestation access!"
    cd "${SCRIPT_DIR}"
    rm -rf "${TEST_DIR}"
    exit 0
else
    ...
    echo ""
    echo "FAILURE: Verification should have passed!"
    exit 1
fi
